...
use near_sdk::{env, AccountId, Promise, NearToken, log};
...
const MIN_BALANCE: U128 = U128(200 * 10_000_000_000_000_000_000); // 200 * 1e19 as u128
...
impl StorageManagement for Contract {
    ...
    fn storage_deposit(
        ...
        if self.accounts.contains_key(&account_id) {
            ...
            // Perform a refund
            if let Some(refund) = amount.checked_sub(min_balance) {
                if refund.as_yoctonear() > 0 {
                    Promise::new(env::predecessor_account_id()).transfer(refund);
                }
            } else {
                env::panic_str("Refund overflow");
            }
        }
    ...
    fn storage_balance_bounds(&self) -> StorageBalanceBounds {
        ...
        let required_storage_balance =
            u128::from(self.bytes_for_longest_account_id) * env::storage_byte_cost().as_yoctonear();
    ...
    // Helper function to get the storage balance of an account
    fn internal_storage_balance_of(&self, account_id: &AccountId) -> Option<StorageBalance> {
        self.accounts.get(account_id).map(|balance| StorageBalance {
            total: U128(balance),
            available: if balance > MIN_BALANCE.0 {
                U128(balance- MIN_BALANCE.0)
            } else { U128(0) },
        })
    }
